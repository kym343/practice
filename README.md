# Solutions for Algorithm Problem
Algorithm Coding Practice

## Problems & Solutions


## [SW Expert Academy](https://swexpertacademy.com/main/main.do)

| # | Title | Solution | Basic idea (One line) |
|---| ----- | -------- | --------------------- |
| 1 | [D4_7829](https://swexpertacademy.com/main/code/problem/problemDetail.do?contestProbId=AWtInr3auH0DFASy&categoryId=AWtInr3auH0DFASy&categoryType=CODE) | C++ |1. Quick sort<br>2. Judgement by Case
<br>

## [Beakjoon Algorithm](https://www.acmicpc.net/)
| # | Title | Solution | Basic idea (One line) |
|---| ----- | -------- | --------------------- |
|1|[1476](https://www.acmicpc.net/problem/1476)| C++ | If Statsment |
|2|[1929](https://www.acmicpc.net/problem/1929)| C++ | Sieve of Eratosthenes |
|3|[1934](https://www.acmicpc.net/problem/1934)| C++ | Repeat dividing two numbers |
|4|[1978](https://www.acmicpc.net/problem/1978)| C++ |1. Divide case 1, 2 or 3, more<br>2. Find the remainder from 3 to n-1 for n |
|5|[2309](https://www.acmicpc.net/problem/2309)| C++ |1. Subtract 100 from total sum<br>2. Find the case with the subtracted value |
|6|[2609](https://www.acmicpc.net/problem/2609)| C++ | Find the remainder from 2 to two nums min -1 |
|7|[6588](https://www.acmicpc.net/problem/6588)| C++ | Sieve of Eratosthenes |
|8|[9613](https://www.acmicpc.net/problem/9613)| C++ |1. Find the remainder from 2 to two nums min -1<br>2. Total sum |
|9|[10430](https://www.acmicpc.net/problem/10430)| C++ | Caculation of modulo |
<br>

## [Inflean ](https://www.inflearn.com/course/%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98/dashboard/)
| # | Title | Solution | Basic idea (One line) |
|---| ----- | -------- | --------------------- |
|001|1부터 N까지 M의 배수합| C++ |  |
|002|자연수의 합| C++ |  |
|003|진약수의 합| C++ |  |
|004|나이 차이| C++ |  |
|005|나이계산| C++ |  |
|006|숫자만 추출| C++ |  |
|7|영어단어 복구| C++ |  |
|8|올바른 괄호| C++ |  |
|9|모두의 약수| C++ |  |
|10|자릿수의 합| C++ |  |
|11|숫자의 총 개수(small)| C++ |  |
|12|숫자의 총 개수(large)| C++ |  |
|13|가장 많이 사용된 자릿수| C++ |  |
|14|뒤집은 소수| C++ |  |
|15|소수의 개수| C++ |  |
|16|Anagram(아나그램 : 구글 인터뷰 문제)| C++ |  |
|17|선생님 퀴즈| C++ |  |
|18|층간소음| C++ |  |
|19|분노 유발자| C++ |  |
|20|가위 바위 보| C++ |  |
|21|카드게임| C++ |  |
|22|온도의 최대값| C++ |  |
|23|연속 부분 증가수열| C++ |  |
|24|Jolly Jumpers| C++ |  |
|25|석차 구하기| C++ |  |
|26|마라톤| C++ |  |
|27|N!의 표현법| C++ |  |
|28|N!에서 0의 개수| C++ |  |
|29|3의 개수는?(small)| C++ |  |
|30|3의 개수는?(large)| C++ |  |
|31|탄화수소 질량| C++ |  |
|32|선택정렬| C++ |  |
|33|3등의 성적은?| C++ |  |
|34|버블정렬| C++ |  |
|35|Special Sort(구글 인터뷰)| C++ |  |
|36|삽입정렬| C++ |  |
|37|Least Recently Used(카카오 캐시 문제 변형)| C++ |  |
|38|Inversion Sequence| C++ |  |
|39|두 배열 합치기| C++ |  |
|40|교집합(투포인트 알고리즘)| C++ |  |
|41|연속된 자연수의 합| C++ |  |
|42|이분검색| C++ |  |
|43|뮤직비디오(이분검색 응용)| C++ |  |
|44|마구간 정하기(이분검색 응용)| C++ |  |
|45|공주 구하기| C++ |  |
|46|멀티태스킹(카카오 먹방 문제 변형)| C++ |  |
|47|봉우리| C++ |  |
|48|각 행의 평균과 가장 가까운 값| C++ |  |
|49|블록의 최댓값| C++ |  |
|50|영지(territory) 선택 : (small)| C++ |  |
|51|영지(territory) 선택 : (large)| C++ |  |
|52|Ugly Numbers| C++ |  |
|53|K진수 출력| C++ |  |
|54|올바른 괄호(stack)| C++ |  |
|55|기차운행(stack 응용)| C++ |  |
|56|재귀함수 분석| C++ |  |
|57|재귀함수 이진수 출력| C++ |  |
|58|이진트리 깊이우선탐색(DFS)| C++ |  |
|59|부분집합(DFS)| C++ |  |
|60|합이 같은 부분집합(DFS : 아마존 인터뷰)| C++ |  |
|61|특정 수 만들기(DFS : MS 인터뷰)| C++ |  |
|62|병합정렬| C++ |  |
|63|인접행렬(가중치 방향그래프)| C++ |  |
|64|경로 탐색(DFS)| C++ |  |
|65|미로탐색(DFS)| C++ |  |
|66|경로 탐색(DFS : 인접리스트 방법)| C++ |  |
|67|최소비용(DFS : 인접행렬)| C++ |  |
|68|최소비용(DFS : 가중치 방향그래프 인접리스트)| C++ |  |
|69|이진트리 넓이우선탐색(BFS)| C++ |  |
|70|그래프 최단거리(BFS)| C++ |  |
|71|송아지 찾기(BFS : 상태트리탐색)| C++ |  |
|72|공주 구하기(Queue : 큐)| C++ |  |
|73|최대힙(priority_queue : 우선순위 큐)| C++ |  |
|74|최소힙(priority_queue : 우선순위 큐)| C++ |  |
|75|최대 수입 스케쥴(priority_queue 응용)| C++ |  |
|76|이항계수(메모이제이션)| C++ |  |
|77|친구인가? (Union&Find 자료구조)| C++ |  |
|78|원더랜드(Kruskal MST 알고리즘 : Union&Find 활용)| C++ |  |
|79|원더랜드(Prim MST 알고리즘 : priority_queue 활용)| C++ |  |
|80|다익스트라 알고리즘| C++ |  |
|81|벨만-포드 알고리즘| C++ |  |
|82|순열구하기(DFS : Depth First Search)| C++ |  |
|83|복면산 SEND+MORE=MONEY (MS인터뷰)| C++ |  |
|84|휴가(삼성 SW역량평가 기출문제 : DFS활용)| C++ |  |
|85|수식만들기(삼성 SW역량평가 기출문제 : DFS활용)| C++ |  |
|86|피자 배달 거리(삼성 SW역량평가 기출문제 : DFS활용)| C++ |  |
|86_0|조합구하기| C++ |  |
|87|섬나라 아일랜드(BFS 활용)| C++ |  |
|88|미로의 최단거리 통로(BFS 활용)| C++ |  |
|89|토마토(BFS 활용)| C++ |  |
|90|라이언 킹 심바(삼성 SW역량평가 기출 : BFS활용)| C++ |  |
|91_0|STL map 자료구조 활용| C++ |  |
|101|네트워크 선 자르기(Bottom-Up)| C++ |  |
|102|최대 부분 증가수열| C++ |  |
|103|최대 선 연결하기| C++ |  |
|104|가장 높은 탑 쌓기| C++ |  |
|105|알리바바와 40인의 도둑(Bottom-Up & Top-Down)| C++ |  |
|106|가방문제(냅색 알고리즘)| C++ |  |
|107|동전교환| C++ |  |
|108|최대점수 구하기(냅색 알고리즘)| C++ |  |
|109|플로이드 워샬 알고리즘| C++ |  |
|100|회장뽑기(플로이드-워샬 응용)| C++ |  |
|101|위상정렬(그래프 정렬)| C++ |  |

<br>

## Others
| # | Title | Solution | Basic idea (One line) |
|---| ----- | -------- | --------------------- |
| 1 | Color Paper | C++ | - |
